 ...
 *
 */
this.ckan.module('odm_autocomplete', function (jQuery) {
  return {
    ...
    getCompletions: function (string, fn) {
      ...
      source = source.replace('lang=en', 'lang=' + $('html').attr('lang'))
    ...
    pythonListToArray: function(s){
      /* sigh. if we were getting a json list,... but it's generally [u'foo', u'bar'] */
      s = s.trim()
      s = s.substring(1, s.length -1)
      return s.split(',').map(function(elt) {
        elt = elt.trim()
        if (elt && elt[0] == 'u'){
          elt = elt.substring(1)
        }
      ...
      if (this.options.tags) {
        if (value.indexOf('[') != -1 && value.indexOf(']') != -1) {
          try {
            ...
            values = this.pythonListToArray(value)
          }
          formatted = jQuery.map(values, this.formatTerm);
        } else {
